 ...
 * @Date: 2018/11/22/022 15:26
 * @Description:
 */
public class ImageUtil {

    public static void createPic(HttpServletResponse response) throws IOException {
        /**
         * 向浏览器输出一张验证码图片
         */
        ...
        //2.建立一个BufferedImage对象，并且设置图片的长和宽
        int width=120;
        int height=30;
        BufferedImage image=new BufferedImage(width, height,BufferedImage.TYPE_INT_RGB);
        //3.获取一个graphics对象用来绘制图片
        ...
        for (int i = 0; i < 4; i++) {
            ...
            g.rotate(r*1.0/180*Math.PI, 5+i*30, 22);
            g.drawString(base.charAt(randomNum(0,base.length()-1))+"", 5+i*30, 22);
